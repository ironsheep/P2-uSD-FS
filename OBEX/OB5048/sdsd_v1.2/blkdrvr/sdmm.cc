...
// 31 Mar 2025:  Bug fix of pin DIR management, for allowing sharing across multiple cogs
...
#ifdef _smartpins_mode_eh
    _drvl(PIN_CLK);    // smartpin enable, added to support multi-cog operation
...
//		wrfast	#0, buff
		drvl	PIN_DO
		drvl	PIN_DI    // enable tx smartpin, DI pin stays high while not transmitting
		mov	bc2, bc  wz
...
void deselect (void)
{
    ...
    _flth(PIN_SS);    // disable SPI interface
    _fltl(PIN_CLK);    // disable clock smartpin
    _fltl(PIN_DI);    // disable tx smartpin
...
#ifdef _DEBUG
    __builtin_printf("sdmm1_open: using pins: %d %d %d %d\n", pclk, pss, pdi, pdo);
#endif    
