...
uint16_t  ledpin;    // pin number of activity LED (-1 == not defined), in upper 8 bits
                     // doubles up as SDSC byte addressing flag in bit0, autodetected
                     // and block read CRC enable flag in bit1, default set, overridden by IOCTL
                     // and clock polarity flag in bit2, autodetected
                     // and SDHC caching flag in bit3, autodetected, code removed (no benefit without queuing)
...
static int  tx_datablock(    // $00 = busy timeout, $3F = no CRC response, %10_101_1 = CRC mismatch, %10_010_1 = CRC matched
    ...
    __attribute__(opt(no-fast-inline-asm))    // use the full assembler, for RES/DAT fitting (C locals also duplicated)
{
    void *ptr = &txblkset;
    ...
    uint32_t CLK_POL )
{
    ...
    uint32_t  cpdiv = (CLK_POL ? clkdiv : clkdiv/2);
    ...
    txblkset.m_align = X_IMM_32X1_1DAC1 | 5 + CLK_REG - CMDAT_REG + clkdiv + cpdiv;  // start(S)-bit is ahead of first streamer bit
    txblkset.m_align2 = X_IMM_32X1_1DAC1 | 5 + CLK_REG - CMDAT_REG + cpdiv;    // no S-bit, CRC abuts end of data block
...
//   The following streamer setup begins its sampling ahead of the second data bit arriving.
		...
		waitse2   wc    // wait for DAT0 low, sets C upon time-out                                      a b  c d
...
if_nc_and_z	call	#crc_check    //                                                                        | c
	...
	if_c	mov	blocks, #2    // indicate a time-out
		ret
...
skip2		res 1    // SKIPF pattern, ignore CRC or process CRC
...
cogdatbuf	res 512/4    // longwords for data block
cogcrcbuf	res 16/8    // longwords for CRC nibbles, must be contiguous with the data block
    }
...
#endif
            if( !wait_ready(_clockfreq()/4) )    // 250 ms, examine busy status on DAT0
                res = RES_ERROR;
            give_pins();
            break;
