...
**Derived from**: Three Claude instances' failure patterns, friction documentation, and systematic violation analysis
...
mcp__todo-mcp__todo_update task_id:"#N" sequence:15  # Order within priority
# If related: add dependency notation in content or tags
```

### 2. Interface Parameter Confusion Anti-Pattern
**Pattern**: Using wrong parameter formats causing systematic failures

**Examples**:
- Using position_id when task_id would be more reliable
- Task IDs displayed as «#49» in output but functions need `"#49"` (string with quotes)
- String/number type mismatches (`estimate_minutes:"60"` vs `60`)

#### Enforcement Mechanisms

**Dual-Parameter Functions** (all 9 support both position_id and task_id):
```bash
# These functions accept EITHER parameter:
todo_start, todo_pause, todo_resume, todo_complete,
todo_update, todo_delete, todo_estimate,
todo_tag_add, todo_tag_remove

# PREFER task_id for reliability (permanent IDs don't change)
mcp__todo-mcp__todo_start task_id:"#49"       # Reliable
...
✓ estimate_minutes is NUMBER not string
✓ priority is lowercase string
✓ status is exact lowercase string
✓ force is BOOLEAN not string
✓ task_id includes # in quotes: "#49"
✓ position_id is plain number: 1
✓ tags is array of strings: ["tag1","tag2"]
```

### 3. Task State Persistence Anti-Pattern
**Pattern**: Trusting inconsistent task states leading to data integrity issues

**Examples**:
- Started tasks showing as "not_started" during completion
- Completed tasks reappearing as incomplete  
- Position/ID mapping confusion causing wrong task operations

#### Enforcement Mechanisms

**Defensive Validation Protocol**:
```bash
# ALWAYS validate before risky operations
Pre-operation checks:
1. mcp__todo-mcp__todo_list  # Get current state
2. Verify target task exists and has expected status
3. If dual-parameter function, validate both position and ID match
4. Document expected change for verification

Post-operation verification:
1. Refresh task list
2. Confirm operation succeeded 
3. If unexpected result, document friction and attempt recovery
```

**Ultra-Safe Backup Pattern**:
```bash
# Before ANY state-changing bulk operation
mcp__todo-mcp__project_dump include_context:true
# Note backup filename for potential rollback

# After operation, verify success
if (operation_failed OR unexpected_result) {
    mcp__todo-mcp__project_restore file:"backup.json" mode:"replace"
    DOCUMENT friction in friction log
...
### 4. Context Bloat Crisis Anti-Pattern
...
### 5. Dual System Integration Chaos Anti-Pattern  
...
### 6. Empty State Trust Anti-Pattern
...
### 7. Manual Context Cleanup Anti-Pattern
